...
pub fn create_router(state: SharedEngineState, exchange: Arc<dyn ExchangeApi>) -> Router {
    Router::new()
        ...
        .route(
            ...
            get(market_handler::get_klines),
        )
        // 策略
        .route("/api/v1/strategy/stages", get(strategy_handler::get_stages))
        .route(
            "/api/v1/strategy/stages/update",
            post(strategy_handler::update_stages),
        )
        .route(
            "/api/v1/strategy/allocate",
            post(strategy_handler::allocate_funds),
        )
        // 手动交易
        .route(
            "/api/v1/trade/order",
            post(trade_handler::place_manual_order),
        )
        .route(
            "/api/v1/trade/close_all",
            post(trade_handler::close_all_positions),
        )
        .route(
            "/api/v1/trade/close_order",
            post(trade_handler::close_tracked_order),
        )
        // 前端静态文件
