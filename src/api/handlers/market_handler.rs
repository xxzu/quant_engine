...
pub async fn get_market_prices() -> Json<serde_json::Value> {
    ...
    for fp in &futures_prices {
        ...
        // 查找24hr变动
        let ticker = tickers_24hr.iter().find(|t| t["symbol"].as_str() == Some(symbol));
        ...
        let volume = ticker
            ...
            .unwrap_or("0");
        let high = ticker
            .and_then(|t| t["highPrice"].as_str())
            .unwrap_or("0");
        let low = ticker
            .and_then(|t| t["lowPrice"].as_str())
            .unwrap_or("0");
...
/// 获取K线数据 (供前端图表使用)
pub async fn get_klines(Path((symbol, interval)): Path<(String, String)>) -> Json<serde_json::Value> {
    let client = reqwest::Client::new();
    let url = format!(
